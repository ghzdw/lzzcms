 ...
 */
public class PreDoc extends SimpleTagSupport{
	@Override
	public void doTag() throws JspException, IOException {
		...
		if (s!=0) {//内容页面
			...
			sbBuffer.append(" select comm_title,comm_htmlpath  from lzz_commoncontent where comm_id<? and column_id=? order by comm_id desc limit 0,1");
			...
			throw new RuntimeException("pre不允许在非内容页使用");
		}
	}
}
